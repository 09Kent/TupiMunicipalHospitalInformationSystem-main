...
$limit = 10; // Exactly 10 per page as required!
$offset = ($page - 1) * $limit;

// Database Query with LIMIT 10 OFFSET
$patients = $patientModel->getPaginated($limit, $offset, $search, $category, $status);
$totalPatients = $patientModel->countTotal($search, $category, $status);
$totalPages = max(1, ceil($totalPatients / $limit));

$activeNav = 'patients';
$pageTitle = 'Patients Directory | Tupi Municipal Hospital Information Management System';
    ...
    <form method="GET" action="" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
      ...
      <div class="flex items-center gap-2">
        ...
        <?php if (!empty($search) || !empty($category) || !empty($status)): ?>
          <a href="<?= base_url('views/patients/index.php') ?>" class="px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl text-xs font-bold transition" title="Reset Filters">
