...
if ($patientId <= 0) {
    header('Location: ' . base_url('views/patients/index.php'));
    exit;
}
...
$historyModel = new PatientHistory();

$patient = $patientModel->getFullProfile($patientId);
if (!$patient) {
    die("<h1>Patient Not Found</h1>");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($dob)) $errors['DateOfBirth'] = 'Date of birth is required.';
    if (empty($phone)) $errors['ContactNumber'] = 'Contact number is required.';
    if (empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) $errors['Email'] = 'Valid email is required.';
    if (empty($address)) $errors['Address'] = 'Address is required.';

    if (empty($errors)) {
        // Update main record
        $patientModel->update($patientId, $_POST);
        ...
        if (!empty($_POST['ContactName'])) {
            $emgModel->update($patientId, [
                ...
                'ContactNumber' => $_POST['EmergencyPhone'] ?? $phone
            ]);
        }

        // Update medical history
        $medModel->update($patientId, [
            'Allergies'               => $_POST['Allergies'] ?? 'None reported',
            'ExistingConditions'      => $_POST['ExistingConditions'] ?? 'None reported',
            'CurrentMedications'      => $_POST['CurrentMedications'] ?? 'None reported',
            'PreviousHospitalization' => $_POST['PreviousHospitalization'] ?? 'None reported'
        ]);
        ...
        $historyModel->log(
            ...
            "Patient record updated by " . (Session::getCurrentUser()['name'] ?? 'Registrator') . '.',
            $userId
        );

        header('Location: ' . base_url('views/patients/view.php?id=' . $patientId . '&updated=1'));
        exit;
    }
}

$activeNav = 'patients';
$pageTitle = 'Edit Patient: ' . e($patient['FirstName'] . ' ' . $patient['LastName']);
  ...
  <div class="flex items-center justify-between pb-3 border-b border-slate-200/80">
    <div>
      <div class="flex items-center gap-2 text-xs font-semibold text-slate-400 mb-1">
        <a href="<?= base_url('views/patients/index.php') ?>" class="hover:text-blue-600">Patients</a>
        <span>/</span>
