...
$totalAppointments = $appModel->countTotal($date, $doctor, $status);
$totalPages = max(1, ceil($totalAppointments / $limit));

$allDoctors = $doctorModel->getAll();

$activeNav = 'appointments';
$pageTitle = 'Appointments & Consultations | Tupi Municipal Hospital Information Management System';
    ...
    <form method="GET" action="" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
      ...
      <div class="flex items-end gap-2">
        ...
        <?php if (!empty($date) || !empty($doctor) || !empty($status)): ?>
          <a href="<?= base_url('views/appointments/index.php') ?>" class="px-3 py-2 bg-slate-100 text-slate-600 rounded-xl text-xs font-bold" title="Clear">
